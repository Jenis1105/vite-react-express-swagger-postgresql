...
const Login = () => {
  ...
  const [password, setPassword] = useState<string>(""); // State to hold the password
  ...
  const user = useSelector((state:RootState) => state.user.user); // get user email from redux store to check if user already loggoed in or not
  const token = useSelector((state:RootState) => state.user.token); // get user token from redux store to check if user already loggoed in or not
